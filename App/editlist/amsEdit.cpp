...
void CAMSEdit::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
LRESULT CAMSEdit::OnCut(WPARAM wparam, LPARAM lparam)
{
	int nStart, nEnd;
	GetSel(nStart, nEnd);
    CString str;
...
LRESULT CAMSEdit::OnClear(WPARAM wparam, LPARAM lparam)
{
	int nStart, nEnd;
	GetSel(nStart, nEnd);
...
LRESULT CAMSEdit::OnPaste(WPARAM wparam, LPARAM lparam)
{
	int nStart, nEnd;
	GetSel(nStart, nEnd);
...
LRESULT CAMSEdit::OnSetText(WPARAM wparam, LPARAM lparam)
{
...
void CAMSEdit::MaskedBehavior::_OnChar(UINT uChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (!_istprint(c)) //控制符
	{
		...
		{//移到下一个有效位
		 ...
		 m_pEdit->ReplaceSel(CString((char)VK_SPACE), TRUE);
...
// Cuts the current selection into the clipboard.
LRESULT CAMSMaskedEdit::OnCut(WPARAM wParam, LPARAM lParam)
{
	int nStart, nEnd;
	GetSel(nStart, nEnd);
    CString str = _T("");
...
LRESULT CAMSDateEdit::OnPaste(WPARAM wParam, LPARAM lParam)
{
	return _OnPaste(0, 0);
}
...
void CAMSMultiMaskedEdit::SetMask(const CString& strMask)
{
	...
	{
		m_pCurrentBehavior = (AlphanumericBehavior*)this;
		((AlphanumericBehavior*)m_pCurrentBehavior)->SetMaxCharacters(nLen);
		return;
	}
...
void CAMSEdit::OnMouseMove(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
...
void CAMSEdit::OnSetFocus(CWnd* pOldWnd) 
{
...
void CAMSEdit::OnKillFocus(CWnd* pNewWnd) 
{
	CEdit::OnKillFocus(pNewWnd);
...
{
	CAMSEdit::OnKillFocus(pNewWnd);
	
	// TODO: Add your message handler code here
	
}

void CAMSNumericEdit::OnMouseMove(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
	
		CAMSEdit::OnMouseMove(nFlags, point);
}

void CAMSNumericEdit::OnSetFocus(CWnd* pOldWnd) 
{
	CAMSEdit::OnSetFocus(pOldWnd);
	
	// TODO: Add your message handler code here
	
}

void CAMSNumericEdit::OnNcPaint() 
{
	// TODO: Add your message handler code here
	
	// Do not call CAMSEdit::OnNcPaint() for painting messages
    CAMSEdit::OnNcPaint();
}
